...
*  @ModifiedBy   
*  @Copyright  asiainfo  
*  @date  2016年2月19日 下午2:58:46
*   创建Employee类的 5 个对象，并把这些对象放入 TreeSet 集合中
*   （下一章：TreeSet 需使用泛型来定义）分别按以下两种方式对集合中的元素进行排序，并遍历输出：
	1). 使Employee 实现 Comparable 接口，并按 name 排序
	2). 创建 TreeSet 时传入 Comparator对象，按生日日期的先后排序。
	提示：Employee类是否需要重写equals()方法？MyDate类呢？
 */
public class TestEmployee {
	//定制排序：对存放于TreeSet中的Employee1类型的元素按照时间由大到小的进行排序
	@Test
	public void test2(){
		Comparator com = new Comparator(){
			...
			public int compare(Object o1, Object o2) {
				if(o1 instanceof Employee1 && o2 instanceof Employee1){
					...
					if(birth1.getYear().intValue()!=birth2.getYear().intValue()){
						return birth1.getYear() - birth2.getYear();
					}else{
						if(birth1.getMonth().intValue()!=birth2.getMonth().intValue()){
		...
		Employee1 e1 = new Employee1("刘德华", 55, new MyDate(4, 11, 1976));
		Employee1 e2 = new Employee1("郭富城", 43, new MyDate(7, 7, 1954));
	...
	//	Employee e6 = new Employee("李敏镐", 65, new MyDate(4, 21, 1945));
